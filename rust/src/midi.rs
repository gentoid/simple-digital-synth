...
pub struct RunningStatus {
    message_kind: MidiMessageKind,
    midi_channel: MidiChannel,
    bytes_counter: u32,
}

impl RunningStatus {
    pub fn new(message_kind: MidiMessageKind, midi_channel: MidiChannel) -> Self {
        Self {
            message_kind,
            midi_channel,
            bytes_counter: 0,
    ...
    pub fn process_midi_byte(&mut self, byte: u8) {
        if byte & 0x80 == 0 {
            self.message_kind
                .process_data_byte(byte, &self.bytes_counter);
            self.bytes_counter += 1;
            return;
        }

        self.midi_channel = MidiChannel::from_byte(&byte);
        self.message_kind = MidiMessageKind::from_byte(&byte);
